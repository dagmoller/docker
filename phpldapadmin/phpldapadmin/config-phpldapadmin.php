...
if (${userTimezone})
	$config->custom->appearance['timezone'] = '${LDAPADMIN_TIMEZONE}';
...
$servers->setValue('server','name','${LDAPADMIN_SERVER_NAME}');
...
$servers->setValue('server','host','${LDAPADMIN_SERVER_ADDRESS}');

/* The port your LDAP server listens on (no quotes). 389 is standard. */
$servers->setValue('server','port',${LDAPADMIN_SERVER_PORT});

/* Array of base DNs of your LDAP server. Leave this blank to have phpLDAPadmin
   auto-detect it for you. */
// $servers->setValue('server','base',array(''));
if (${userBaseDN})
	$servers->setValue('server','base',array(${LDAPADMIN_BASEDN}));
...
/* Five options for auth_type:
   ...
   session['blowfish']. */
$servers->setValue('login','auth_type','${LDAPADMIN_AUTH_TYPE}');
